...
const AluminumPage = () => {
  ...
  const fetchMaterials = async () => {
    try {
      const response = await api.get('/rawmaterial/category/ALUMINIUM');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) { await api.put(`/rawmaterial/${editingId}`, materialData); toast.success('Updated! ✅'); }
      else { await api.post('/rawmaterial', materialData); toast.success('Added! ✅'); }
      fetchMaterials(); resetForm();
  ...
  const handleDelete = async (id) => {
    ...
    if (!confirmed) return;
    try { await api.delete(`/rawmaterial/${id}`); toast.success('Deleted! ✅'); fetchMaterials(); }
    catch (error) { toast.error('Failed'); }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Aluminum Materials</h1>
