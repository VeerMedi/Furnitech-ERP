...
const HardwarePage = () => {
  ...
  const fetchMaterials = async () => {
    try {
      const response = await api.get('/rawmaterial/category/HARDWARE');
  ...
  const handleDelete = async (id) => {
    const confirmed = await confirm('Delete this material?', 'Delete');
    if (!confirmed) return;
    try {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Hardware Materials</h1>
