...
const importFromExcelIntelligent = async (req, res) => {
    try {
        ...
        const columnPatterns = {
            name: ['name', 'item', 'product', 'material', 'description', 'title', 'items', 'item name'],
            category: ['category', 'type', 'class', 'group', 'categories'],
            brand: ['brand', 'manufacturer', 'make', 'company'],
            thickness: ['thickness', 'thick', 'thk'],
            width: ['width', 'breadth', 'w'],
            length: ['length', 'long', 'size', 'l'],
            color: ['color', 'colour', 'shade', 'clr'],
            finish: ['finish', 'finishing', 'surface', 'fnsh'],
            grade: ['grade', 'quality', 'grd'],
            uom: ['uom', 'unit', 'measure', 'um', 'units'],
            costPrice: ['cost', 'price', 'rate', 'purchase', 'cp', 'amount', 'value', 'rs', 'inr'],
            sellingPrice: ['selling', 'sale', 'mrp', 'sp', 'retail'],
            currentStock: ['stock', 'quantity', 'qty', 'available', 'avl', 'bal', 'balance'],
        ...
        headerRow.forEach((header, index) => {
            ...
            for (const [field, keywords] of Object.entries(columnPatterns)) {
                ...
                if (bestScore === 100) break; // Stop if exact match found
            }
        ...
        const parseCategory = (val) => {
            ...
            const map = {
                PANEL: ['PANEL', 'BOARD', 'PLYWOOD', 'HBD', 'MDF', 'PLN', 'HDF', 'PLY'],
                LAMINATE: ['LAMINATE', 'LAMINATION', 'LAM'],
                EDGEBAND: ['EDGEBAND', 'EDGE'],
                HARDWARE: ['HARDWARE', 'FITTING', 'FITTINGS'],
                GLASS: ['GLASS'],
                FABRIC: ['FABRIC', 'CLOTH'],
            ...
            // If val contains "TABLE" or "FURNITURE", categorize as PANEL
            if (v.includes('TABLE') || v.includes('FURNITURE')) return 'PANEL';
            return 'OTHER';
        };
